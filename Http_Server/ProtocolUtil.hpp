...
class Http_Request{
    ...
    private:
        ...
        bool Search_cgi;
    public:
        Http_Request():blank("\r\n"),path(WEBROOT),cgi(false),Search_cgi(false)
    {}
        ...
        void UriParse()
        {
            ...
            else if(method=="POST" && suffix!=".html")
            {
                cgi=true;
                path+=uri;
            }
            else if(method=="GET" && suffix==".html")
            {
                pos=uri.find('?');
                if(pos!=std::string::npos)
                {
...
/////////////////////////////////////////////////运行CGI程序
        ...
        static int ProcessSearchCgi(Connect *con, Http_Request *rq, Http_Response *rsp) 
        {}
#endif
#if 0
        static int ProcessHtmlCgi(Connect *con, Http_Request *rq, Http_Response *rsp)
        {
            int output[2];
            pipe(output);
            std::string html_bin=rsp->GetPath();
            ...
            else if(pid==0)
            {
                ...
                dup2(output[1],1);
                execl("/bin/html", "html", html_bin.c_str(), NULL);
                exit(1);
            }
            else
            {   
...
//////////////////////////////////////////////运行CGI程序
        ...
        static int ProcessResponse(Connect *con, Http_Request *rq, Http_Response *rsp)
        {
            ...
            else if(rq->IsSearchCgi())
            {
                LOG("MakeResponse Use SearchCgi!", NORMAL);
                ProcessSearchCgi(con, rq, rsp);
            }
            else
            {
